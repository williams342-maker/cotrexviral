...
- 2026-02-28 (part 126) **🎯 P0 — Cortex on production: REAL root cause found & fixed**
  ...
  - **RCA**: `cortex/memory.py` lazy-loads `fastembed` (BAAI/bge-small-en-v1.5, 130MB ONNX) on the first call into `recall_semantic` / `record_turn`. On a 1Gi pod that's a hard OOM. The pod gets killed mid-stream → EventSource sees its connection drop → "Connection closed before completion". My previous heartbeat fix couldn't save it because heartbeats don't help when the pod itself dies. This explains every observed symptom: instant fail (mid-`recalling` phase), no useful error, works perfectly in preview (no resource caps), only fails the chat endpoint (because only the chat path touches Qdrant).
  - **Fix** in `cortex/memory.py`: gated the entire Qdrant + fastembed code-path behind a new env var `CORTEX_VECTOR_MEMORY`. **Defaults to DISABLED** so production stays alive on a 1Gi pod. When disabled, `_get_qdrant()` returns `None` immediately, `recall_semantic()` returns `[]`, `record_turn()` becomes a no-op — Cortex chat keeps working end-to-end with Mongo-backed strategic memory + conversation history. Semantic recall is the only feature that degrades. Set `CORTEX_VECTOR_MEMORY=enabled` in env to re-enable on bigger pods or after migrating to a managed vector DB.
  - **Preview kept enabled**: added `CORTEX_VECTOR_MEMORY=enabled` to `/app/backend/.env` so semantic recall still works locally. Verified: `recalled_count: 5` returned, full chat stream completes in 15.4s with rich strategy summary + recalled previews intact.
  - **Production verified clean**: with `CORTEX_VECTOR_MEMORY` unset (default-disabled), `recall_semantic()` returns `[]`, no Qdrant init attempted, no fastembed import triggered, `get_strategy()` (Mongo) still works.
  - **Bonus fixes from the same audit**:
    - `backend/.env`: quoted `MAILGUN_FROM` and `MAILTRAP_FROM` values (they contained `<>` characters that K8s env-var parsing was choking on).
    - Added `GET /api/health` returning `{status, vector_memory_enabled, time}` — quick prod diagnostic, no auth required. Use it after deploy to confirm the env flag is wired correctly: `curl https://cortexviral.com/api/health` should return `vector_memory_enabled: false`.
  - **Lint clean, end-to-end verified in preview**: full Cortex stream completes with all 5 phases, semantic recall returns 5 hits, no errors.
  - **Long-term path (Future/Backlog)**: replace local fastembed with a managed embeddings API (Emergent LLM key likely supports embeddings via `emergentintegrations`, or OpenAI/Voyage). That would let semantic recall work on production without growing the pod.
...
- 2026-02-28 (part 119) **🎯 Mission Dashboard — Recommended Action hero (P2)**
  ...
  - **Mission Dashboard hero** (`Missions.jsx` → new `<RecommendedActionHero />`): violet/blue gradient card with cortex-grid-bg, "CORTEX RECOMMENDS" kicker + source pill ("From analysis" / "From briefing"), title, summary, mini-stats row (confidence · outcome · ETA · cost), gradient confidence bar, and 3 CTAs — **Launch as mission** (POSTs `/cortex/console/execute` with `override_autonomy=3` to force `launch` behavior, fires success toast + reloads missions), **Discuss with Cortex** (navigates to `/dashboard/command-center?prefill=<title>`), **Dismiss** (POSTs `/cortex/plan/cancel` with rec_id=bridge_id, 7-day cooldown, hides hero with toast). Loading skeleton via `data-testid="recommended-action-loading"`. Card hides entirely when `has_recommendation=false`.
  ...
  - **iter30 tests**: 5/5 backend pytest pass (returns 'bridge' source with 95% confidence, requires auth, skips dismissed bridges, falls back to briefing, has_recommendation=false handled) + all frontend flows pass (hero renders with all testids, loading skeleton lifecycle, launch CTA → toast + hide, discuss CTA → prefill + URL cleared, dismiss CTA → cancel POST + toast + hide, hidden when no recommendation). Zero functional bugs. Pytest file at `/app/backend/tests/test_recommended_action.py`.

- 2026-02-27 (part 118) **🪄 Command Center session-management redesign — Fresh-session-every-visit · Cortex greeting · Settings toggle**
  ...
  - **Generic user preferences API** (`routes/auth.py`): new `GET/PUT /api/user/preferences` pair, validates against a `_PREF_VALIDATORS` map (unknown keys → 422, bad values → 422, empty body → 422 — unified per testing-agent code review). Defaults shipped via `_PREF_DEFAULTS` so the frontend never has to special-case unset keys. First key: `conversation_mode ∈ {'fresh_every_visit' (default), 'resume_last'}`.
  ...
  - **Cortex greeting bubble** — empty-state rewritten as a centered, prominent "How can I help you today?" gradient title (text-26-32) with CORTEX kicker badge and explanatory subtitle. Client-rendered ONLY (never persisted), disappears the instant the user sends their first turn. `data-testid: cortex-greeting`.
...
- 2026-02-26 (part 116) **🔐 Production polish batch — signed URLs · WS tickets · DELETE-cancel · Whisper cost · PPTX→Campaign**
  ...
  - **DELETE-during-build cancel** (`cortex/campaign_builder.py`) — `_RUNNING_PIPELINES: dict[str, asyncio.Task]` tracks live build tasks; `cancel_pipeline(cid)` pops + cancels. `_run_pipeline` outer wrapper catches `asyncio.CancelledError` cleanly. DELETE handler returns `{pipeline_cancelled: bool}`. Verified via backend log "campaign pipeline cancelled for {cid}". Stops wasted LLM credits when users delete a building campaign. *Note*: ingress 502 can still appear if DELETE arrives during a long LiteLLM call — that's a separate event-loop-blocking concern (open).
  - **Whisper cost surfacing** (`cortex/asset_extraction.py`) — `_extract_video` now emits `transcription_cost_usd = round(min(duration_s, 300) / 60 * 0.006, 4)` + `transcription_provider="whisper-1"`. Frontend Asset card shows a violet `$0.001` badge (testid: `asset-whisper-cost-{id}`) for transparency. Asset detail thumbnail honors `extraction_meta.thumb_format`.
  - **PPTX → Campaign deep version**:
    - `_extract_pptx` rewritten to also score every slide on (slide-1 bias ×3) + (image area ÷ max ×2) + (title bonus 0.5) + (image count ×0.3). Winning slide rendered to a base64 thumbnail: largest embedded image scaled to ≤480px JPEG (preferred) OR a Pillow-rendered text card PNG placeholder (fallback when the deck has no embedded images). `extraction_meta` now carries `hero_slide_index`, `hero_slide_score`, `hero_image_count`, `thumb_format`.
    - New endpoint `POST /api/cortex/assets/{asset_id}/build-campaign` one-clicks asset → ensure brief → kick off Phase C campaign. Returns `{brief_id, campaign_id, hero_image_url}`. Idempotent on the brief side; always creates a fresh campaign row.
    - Frontend `Assets.jsx` detail view: new **Build Campaign** button (violet, Rocket icon, testid: `asset-build-campaign`) when status='complete' and kind ∈ (pdf, pptx, url, image, video). Click → POST + navigate to `/dashboard/campaigns?id=<new>`.
  - **iter27 tests**: 14/14 backend pytest pass (signed-URL tampering / expiry / per-key isolation; ws-ticket happy path + replay-rejection; DELETE cancel + log assertion; PPTX hero score + thumb populated; build-campaign endpoint 404/409 paths). Frontend 100% (Whisper badge + Build Campaign button verified visually). Zero bugs found; two cosmetic follow-ups (HITL WS warning + signing-secret rotation) — first addressed inline, second tracked as backlog.
...
- 2026-02-26 (part 112) **📤 Phase D — Direct Social Media Publishing (bulk push to calendar)**
  ...
  - **Platform normalization**: brief returns "instagram story" / "Instagram_Story" etc. — normalized via `space→underscore` + `_PLATFORM_ALIASES` so `instagram_story → instagram` before publishing. Frontend mirrors the same normalization in the bulk-push counter.
  - **Idempotency** at the cortex_post level: stamp `cortex_social_posts.pushed_at` after every successful push. Re-running the endpoint returns `skipped: [{reason: 'already_pushed', posts_id: ...}]` for each previously-pushed post — safe to call repeatedly.
  ...
  - **Content composition** (`_compose_content`): joins post `headline` (if not already in body's first 160 chars) → `body` → `hashtags` (auto-prefixed with `#`). CTA stays out of the body — each platform's CTA convention differs (link in bio / first comment / etc).
  ...
  - **Frontend** (`Campaigns.jsx`): new `BulkPushBar` at the top of the Posts tab — emerald gradient surface with counter ("X of Y pushable posts ready · Z already pushed"), Drafts/Schedule mode pills, datetime + cadence inputs (Schedule only), Push button with live count + Loader2 spin during push. Result line shows "Pushed N · Open calendar →" on success. Per-PostCard `Pushed ✓` badge appears whenever `cortex_social_post.pushed_at` is set.
  ...
  - **Bug found + fixed during smoke test**: Claude was occasionally returning nested fields as JSON-encoded strings with unescaped double quotes inside body text (e.g. `"body": "...e.g., "cutting metal" 9 years..."`). Migrated `_coerce_nested()` from a hand-rolled regex repair to the `json_repair` library (0.59.10) — handles all such cases robustly. Also tightened the campaign-builder system prompt: nested fields MUST be native JSON arrays/objects (not strings), and body text MUST use curly quotes (\u201c\u201d) instead of straight double quotes inside string values.
  - **Live verified**: built fresh campaign `3d579a6b...a13cb` (12 posts + 3 emails + 8-section landing + 5 creatives), pushed all 12 as drafts on first call (0 skipped), re-pushed → 0 pushed + 12 already_pushed. Posts visible in `/dashboard/marketing-calendar`.

- 2026-02-26 (part 111) **🚀 Phase C — Autonomous Campaign Builder (brief → posts + emails + landing + creatives in one click)**
  - **Orchestrator** (`cortex/campaign_builder.py`) — `build_campaign()` inserts a `cortex_campaigns` row in status=`building`, kicks off `asyncio.create_task(_run_pipeline(...))`, and returns the row immediately. Pipeline is best-effort: each step (`compose_artifacts` → `persist_posts` → `persist_emails` → `persist_landing_page` → `queue_images`) is wrapped in a try/except that flips the step row to `failed` with the exception text but lets the rest of the pipeline continue. Partial successes still land what they can.
  - **Single-pass LLM compose** — one `compose_campaign_artifacts` tool-call (Claude Sonnet 4.5 via `cortex_tool_call`) produces the full artifact bundle (`campaign_title`, `campaign_goal`, `campaign_summary`, `social_posts`, `email_sequence`, `landing_page`) so brand voice stays cohesive across surfaces. Separate calls per artifact drift.
  - **Defensive coercion** (`_coerce_nested`) — Claude occasionally encodes nested arrays/objects as JSON-encoded strings inside tool-call args. We `json.loads()` (falling back to `ast.literal_eval`) any string-shaped `social_posts`/`email_sequence`/`landing_page` back into the expected dict/list, logging both success (info) and parse failures (warning). Caught and fixed during smoke test — first campaign built without coercion lost emails + landing page.
  - **Image fan-out** — `_queue_images()` re-uses Phase B's `generate_for_concept()` provider abstraction with a `Semaphore(3)` so a brief with 5-6 concepts doesn't hammer Gemini/OpenAI. Per-concept idempotency check skips concepts that already have a complete creative for this brief.
  ...
  - **4 endpoints** (`routes/cortex_campaigns.py`):
    - `POST   /api/cortex/campaigns         {brief_id}`  → create + kick off pipeline (404 if brief missing or not owned).
    - `GET    /api/cortex/campaigns         ?status&limit`  → list newest-first, soft-deletes excluded.
    - `GET    /api/cortex/campaigns/{id}    ?include=posts,emails,landing_page,creatives,brief`  → hydrate everything. **Creatives include fallback**: if the campaign has no creatives of its own, returns brief-level creatives (Phase B may have generated them upstream before the user clicked Build full campaign).
    - `DELETE /api/cortex/campaigns/{id}`  → soft delete (`deleted_at` + `status='deleted'`).
  ...
  - **Live verified end-to-end** (craftersmarket.org brief): campaign "See It Before You Buy It" → 13 platform posts + 3-touch email nurture + 6-section landing page outline + 5 creatives (4 Gemini photoreal + 1 OpenAI typography infographic). Status=complete in ~3 minutes.
  - **Known minor**: DELETE issued while compose_artifacts is in-flight intermittently 502s at ingress (~60-90s) — function eventually lands. Build pipeline already uses `asyncio.create_task`; likely a LiteLLM streaming side-effect. Tracked as P2 for future RCA.
...
- 2026-02-26 (part 107) **⚡ Native Tool-Calling promotion + Mission Detail provenance card**
  - **Tool-call promotion**: `/api/cortex/memory/tool-call-trend` flipped `promotion_ready: true` (95.3% success over 127 calls in 24h, 100% in last 1h). Promoted three remaining `cortex_chat(json_mode=True)` call sites to native `cortex_tool_call(tool=...)`:
    ...
    - **`cortex/memory.py`** — strategy distillation (`update_strategy_summary`) now uses `record_strategy` tool with structured `summary/goals/bottlenecks/recent_themes` schema. Eliminates the JSON-parsing error path that was silently nuking strategy updates on malformed model output.
    - **`cortex/optimization_loop.py`** — OODA LLM-augmented detector (`_llm_rules`) now uses `surface_bottlenecks` tool with array-of-finding schema. The legacy `_parse_llm_findings` (string parser) stays in place for backward-compat tests; new code path uses `_normalize_findings` (dict parser).
...
- 2026-02-26 (part 105) **🔧 Bridge LLM bug fix + cross-feature verification (Action-First × Apply-Recommendation × Bridge)**
  ...
  - **Cross-feature smoke test** (`/tmp/smoke_cross_feature.sh`, 19/19 assertions): verifies parts 95 + 102 + 103 + 104 all work together in a single user session — Action-First `show me the leads` (stage=action), Action-First scan `scan example.com` (creates real analysis_jobs row), Discovery Budget forces advance on turn 3, Bridge synthesis with `llm:claude` source, Optimize-via-Bridge on site_scan creating L3 mission with `auto_optimize_meta.source=bridge`, Pushback regenerate replacing the bridge row, Executive Insights list user-scoped, OODA finding Apply returning result + idempotent on retry.
  - **Regression sweep**: `test_recommendation_bridge.py` (6) + `test_recommendation_bridge_followups.py` (4) + `test_cortex_iter24.py` Action-First (11) — **21/21 pass in 2m22s**. One pre-existing assertion in iter25 was hard-coded to `source in ('llm', 'heuristic')` — updated to accept `llm:*` labels.

- 2026-02-26 (part 104) **🌉 Recommendation Bridge follow-ups — Optimize-via-bridge, Pushback loop, Executive Insights panel**
  - **Optimize Automatically — now job-type-agnostic.** Previously gated to SEO scans only; now every completed analysis (site_scan, competitor_audit, content_audit, seller_discovery) flows through the bridge to spawn an L3 mission. The SEO path keeps the dedicated `seo_auto_fix` drafter; all other types read `bridge.mission_intent` + `bridge.mission_params` and create the right mission shape with `auto_optimize_meta` stamped (source/bridge_id/intent/confidence/recommendation) for full traceability. Idempotent — second call returns the existing mission. **Live verified**: site_scan → L3 `improve_conversions` mission with the bridge's recommendation baked into the title.
  ...
  - **Live verified end-to-end**: real SEO + site_scan jobs → bridges generated → Executive Insights panel showing 3 cards (SEO 75% × 2, SITE 72%) → Optimize Automatically on site_scan returns L3 mission with confidence 72% intent=improve_conversions → pushback regenerate replaces bridge row and stamps pushback text.

- 2026-02-26 (part 103) **🌉 Proactive Recommendation Bridge — the layer between analysis and action**
  - **The missing link**: Every completed analysis now flows through a structured executive recommendation: Scan → Findings → **Executive Insight** → **Recommendation** → **Mission Option** → Optimization. Previously analyses ended at metric tiles; now Cortex always answers "What should I do next?"
  ...
  - **Auto-post into chat**: `analysis_runner._complete()` now fires `asyncio.create_task(_emit_bridge_with_pacing)` — waits 1.6s after the analysis_complete metric-tile message lands, then synthesizes + posts a second Cortex turn with `kind='recommendation_bridge'` carrying the structured payload. User experience: metrics card → 1.6s pause → reasoning prose → recommendation card with confidence bar + 3 CTAs.
  - **3 CTAs honor the "Explain THEN Recommend" rule**: 🔹 `View Findings` (opens report) · 🔹 `Discuss Recommendation` (drops a deeper-reasoning Cortex turn into chat for pushback / refinement) · 🔹 `Create Mission` (one-click mission from the bridge's mission_intent).
  - **4 new endpoints** in `routes/cortex_recommendation_bridge.py`: `GET /cortex/recommendation-bridges/{job_id}` (lazy-generates if missing), `POST /{job_id}/regenerate` (force resynth on pushback), `POST /{job_id}/discuss` (posts deeper Cortex explanation with the 4-part frame), `GET /cortex/recommendation-bridges` (list all for user — feeds future Executive Insights panel).
  - **Frontend** (`RecommendationBridgeCard.jsx` + `ChatMessage.jsx`): violet gradient card with 4-part insight rows (Finding/Root Cause/Recommendation/Impact — each with tone-coded icon), animated confidence meter (color-coded: green ≥80, amber 60-79, rose <60), confidence badge with word descriptor ("High/Moderate/Low confidence"), 3 primary CTAs with Discuss-driven conversation refresh via `cortex:conversation:refresh` event.
  - **Auto-refresh wired**: `ActiveWorkRail.jsx` dispatches `cortex:conversation:refresh` on job status transitions (queued/running → completed/failed) + again at 2.2s to catch the delayed bridge post. `CommandCenter.jsx` listens for the event and reloads thread — bridge appears inline without manual refresh.
  ...
  - **iter25 tests** (`tests/test_recommendation_bridge.py`): 6 tests — synthesis produces all 5 fields, idempotency, only-when-completed gate (queued/running/failed/cancelled return None), chat-turn auto-post with kind+payload, per-job-type heuristic fallback (5 types), confidence clamping (out-of-range/non-int). **6/6 pass in 4.14s**.
  - **Live verified end-to-end**: real SEO scan of `https://example.com` → job completed in 3s → bridge auto-posted to chat → frontend renders the full 4-part card with reasoning prose + confidence bar (75%) + 3 CTAs in the live preview.

- 2026-02-26 (part 102) **🧠 Discovery & Executive-Consultant redesign — Cortex stops interrogating, starts producing value**
  ...
  - **Auto-trigger scans from chat**: typing "scan craftersmarket.org" / "review cortexviral.com" / "audit https://example.com" creates a REAL `analysis_jobs` row immediately, fires the runner, and replies with a job ID. "audit my site" (no URL) → asks for the URL, doesn't fake-scan.
  - **Discovery Budget** (`cortex/stages.py`): tracks discovery turns in `cortex_conversations` history. After 2 rounds, the classifier output is post-processed to FORCE progression to analysis — the system literally cannot return another discovery turn after budget exhausts. Frontend shows `Discovery 1/2` / `2/2` indicator on each clarifying card.
  ...
  - **Answer-Shortcut cards** (`StageComponents.jsx ClarifyingQuestionsCard`): when discovery DOES fire, the classifier now returns `answer_shortcuts` (3-6 short clickable answers). They render as primary pill-button affordances; clarifying questions become secondary text. Clicking a shortcut submits its label as a tagged user message that ADVANCES state — never restarts discovery. Forbidden by `_normalize`: any shortcut containing "?" is dropped (no question-mirroring allowed).
  - **In-chat Action-First cards** (`ChatMessage.jsx ActionDataCard`): when `stage=action`, Cortex's bubble embeds a kind-specific data card — top-category chips + quality tiles for leads, recent-5 list for reports, top-5 list for opportunities, mission list with autonomy levels, "tracking on right rail · Job #abc1234" callout for scans.
  - **Live verified end-to-end**: "grow my business" → discovery with 5 answer shortcuts; "show me the leads" → instant consultative summary with category chips + quality tiles; "scan craftersmarket.org" → real job created with ID, Active Work rail picks up the work, completion message lands back in chat with View Report / Create Mission CTAs.
  - **iter24 tests**: 11 tests covering Action-First routing (show/list/scan), Discovery Budget enforcement (seeded 2 discovery turns → forced advance), Answer Shortcuts (≥3 returned, no '?' mirroring), Action matcher unit tests (10 positive + 4 negative cases), auto-scan with URL extraction including subdomains. **17/17 pass** when iter22+iter24 run together. Full regression: 78/81 (2 skipped, occasional network timeout flake on iter22 mitigated by 45s timeout).
...
- 2026-02-26 (part 99) **🎓 AI-guided onboarding mission — Cortex teaches the platform through conversation**
  ...
  - **Demo mission**: real `missions` row with `demo: true` + `demo_phase_idx`. The orchestrator polls `POST /onboarding/demo-tick` every 2.2s during `mission_lifecycle` + `autonomous_execution` so the user watches phases advance live (Discovery → Qualification → Outreach → Conversations). Active Mission Rail renders these with an amber **DEMO** badge so they're clearly labeled as demo data while onboarding runs. Hard-deleted on `complete` or `skip`.
  - **Frontend orchestrator** (`OnboardingOrchestrator.jsx`): top-right floating card with the actual chat thread still visible underneath. Typewriter animation (25ms/char, click message to fast-forward, respects `prefers-reduced-motion`). Step indicator (`Step N of 7`), animated gradient progress bar (framer-motion), animated brain icon. Spotlight portal (`OnboardingSpotlight.jsx`) frames the targeted region with a violet halo + pulsing glow and dims the rest of the page — but **does not capture clicks** so the user can keep using the UI.
  - **"Show me around" command** — persistent replay handle in the header (`onboarding-replay-btn`), AND typing "show me around" in the composer triggers replay (intercepted before LLM call — saves cost). Sends `{replay: true}` to `/start`, bypasses the eligibility gate.
  - **`/skip` endpoint** stamps `users.onboarded_at` so the gate stays closed permanently. Replay button still works.
  - **iter22 tests**: 6 tests covering eligibility gate (HTTP 409 without replay), state machine traversal, goal capture + personalization, demo mission lifecycle (created → ticked → hard-deleted), skip path, `users.onboarded_at` stamping. **58/58 tests pass** across iter17→iter22 (full regression).
