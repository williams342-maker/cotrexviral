...
- 2026-06-03  **Cloudflare R2 activated as primary asset storage** (P3 done). `ASSET_STORAGE_BACKEND=s3` + R2 creds set in `backend/.env` (bucket `cortexviral-assets`, endpoint `https://<account>.r2.cloudflarestorage.com`, region `auto`). Live e2e round-trip verified: upload via `/api/cortex/assets/upload` → bytes land in R2 → download via `/api/cortex/assets/file/{key}` returns identical bytes → soft-delete via bulk-delete removes the R2 object. All 52 tests across asset/migration/bulk/prompt suites pass with R2 live. HybridStorage fallback still serves the 24 legacy files in Emergent obj store (option (b) full-migrate to R2 pending user confirmation).
