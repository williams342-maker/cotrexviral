          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4">
            ...
            <div className="space-y-2" data-testid="activity-feed">
              {(data?.runs || []).length === 0 ? (
                ...
                (data?.runs || []).map((r) => {
                  const pillCls = r.status === 'completed'
                  ...
                  return (
                    <div key={r.id} className="rounded-lg border border-white/5 bg-zinc-950/50 p-2.5" data-testid={`activity-run-${r.id}`}>
                      <div className="text-xs text-zinc-200 line-clamp-2 leading-snug">{r.brief || '(no brief)'}</div>
                      <div className="flex items-center justify-between mt-1.5 gap-2">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span className={`text-[9px] px-1.5 py-0.5 rounded border shrink-0 ${pillCls}`}>{r.status}</span>
                          {r.skip_distribution && (
                            <span
                      ...
                      {r.status === 'awaiting_approval' && (
                        <div className="mt-2 pt-2 border-t border-amber-500/10 flex items-center gap-1.5">
                          <span className="text-[10px] text-amber-300 flex-1 italic">Distribution paused — your review needed</span>
                          <button
                            type="button"
