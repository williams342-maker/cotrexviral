...
const PHASES = [
  {
    ...
    items: [
      ...
      { name: 'Instagram automation (OAuth + publishing)', status: 'shipped' },
      { name: 'Facebook automation (OAuth + publishing)', status: 'shipped' },
      { name: 'TikTok automation (OAuth + publishing)', status: 'shipped' },
      { name: 'LinkedIn automation (OAuth + publishing)', status: 'shipped' },
      { name: 'Pinterest automation (OAuth + publishing + insights)', status: 'shipped' },
      { name: 'YouTube automation (OAuth + publishing)', status: 'shipped' },
    ],
    notes: [
      'Originally scoped as 2–4 weeks. Actual: ~8 weeks including auth, billing, admin, and 6 OAuth adapters with real publishing dispatch.',
      'Per-post analytics ingestion currently live on Pinterest; TikTok / LinkedIn / Meta / YouTube insights ingestion is on the Phase 2 backlog.',
    ],
  },
  {
    ...
    items: [
      { name: 'Scheduled posting (every platform)', status: 'shipped', note: 'All 6 OAuth adapters dispatch real publishes via /channels/publish + scheduler.py.' },
      ...
      { name: 'AI memory (per-user persistent context)', status: 'shipped', note: 'OpenAI 1536-dim embeddings stored per-user in cortex_memory with recall in every Cortex turn.' },
      { name: 'Social listening', status: 'partial', note: 'Reddit + Google Trends live via trends_engine.py. X (Twitter) on the roadmap.' },
    ],
    notes: [
